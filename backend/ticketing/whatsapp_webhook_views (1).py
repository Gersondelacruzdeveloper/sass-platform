...
def _store_inbound_message(
    *,
    ...
    with transaction.atomic():
        ...
        # Telegram is an optional observer. ``robust=True`` ensures a broker or
        # notification failure can never change the webhook acknowledgement.
        transaction.on_commit(
            lambda message_id=inbound.pk: queue_telegram_monitor_message(message_id),
            robust=True,
        )
