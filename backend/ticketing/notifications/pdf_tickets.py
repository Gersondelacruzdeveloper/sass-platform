...
def _hex_color(value: str, fallback: str = "#111827"):
...
def _get_guest_summary(booking: Any) -> str:
    ...
    infants = getattr(booking, "infants", None)
    total = getattr(booking, "total_guests", None)
...
def _draw_label_value(
    ...
    pdf.setFillColor(_hex_color(label_color, "#6B7280"))
    pdf.setFont("Helvetica-Bold", 7)
    pdf.drawString(x, y, label.upper())

    pdf.setFillColor(_hex_color(value_color, "#111827"))
    pdf.setFont("Helvetica-Bold", 12)
    y -= 5 * mm
    return _draw_wrapped_text(pdf, value, x, y, max_chars=max(18, int(width / 4.2)))


def _draw_logo_or_brand(pdf: canvas.Canvas, branding: dict[str, Any], x: float, y: float):
    ...
    pdf.drawString(x, y - 7 * mm, branding["brand_name"][:34])
...
def generate_ticket_pdf(booking: Any) -> bytes:
    ...
    ticket_h = PAGE_HEIGHT - (margin * 2) - 20 * mm
    ...
    # Page background
    pdf.setFillColor(_hex_color("#F3F4F6"))
    pdf.rect(0, 0, PAGE_WIDTH, PAGE_HEIGHT, fill=1, stroke=0)

    # Main ticket card
    pdf.setFillColor(colors.white)
    pdf.setStrokeColor(_hex_color("#E5E7EB"))
    pdf.roundRect(ticket_x, ticket_y, ticket_w, ticket_h, 7 * mm, fill=1, stroke=1)

    # Header
    header_h = 42 * mm
    header_y = top - header_h
    pdf.setFillColor(_hex_color(primary, "#111827"))
    pdf.roundRect(ticket_x, header_y, ticket_w, header_h, 7 * mm, fill=1, stroke=0)

    # Cover the lower rounded corners of header so only top card corners look rounded
    pdf.rect(ticket_x, header_y, ticket_w, 8 * mm, fill=1, stroke=0)

    _draw_logo_or_brand(pdf, branding, ticket_x + 9 * mm, top - 9 * mm)

    pdf.setFillColor(colors.white)
    pdf.setFont("Helvetica", 9)
    pdf.drawString(ticket_x + 9 * mm, header_y + 10 * mm, "Official booking ticket")
    ...
    pdf.setFillColor(_hex_color(muted, "#6B7280"))
    ...
    pdf.setFillColor(_hex_color(accent, "#F59E0B"))
    pdf.roundRect(content_x, y - 8 * mm, 34 * mm, 8 * mm, 4 * mm, fill=1, stroke=0)
    pdf.setFillColor(colors.white)
    pdf.setFont("Helvetica-Bold", 8)
    pdf.drawCentredString(content_x + 17 * mm, y - 5.3 * mm, _get_payment_status(booking).upper()[:18])

    y -= 18 * mm
    pdf.setFillColor(_hex_color(text_color, "#111827"))
    ...
    grid_top = y - 18 * mm
    col_gap = 7 * mm
    col_w = (ticket_w - (18 * mm) - (2 * col_gap)) / 3
    ...
    # Payment summary
    payment_y = second_row_y - 34 * mm
    pdf.setFillColor(_hex_color("#F9FAFB"))
    pdf.setStrokeColor(_hex_color("#E5E7EB"))
    pdf.roundRect(content_x, payment_y - 24 * mm, ticket_w - 18 * mm, 25 * mm, 4 * mm, fill=1, stroke=1)
